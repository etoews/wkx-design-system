...
| 4 · Namespace | [04-namespace.html](04-namespace.html) | The namespace by night or by day. The console layout and status lights of Harbour Watch, speaking the path grammar: panels addressed by their paths, plan symbols for state, and hover-to-highlight on every token of the ubiquitous language. Light and dark schemes follow the system setting, with a manual override in the nav. Fluid width: the console takes the whole screen, panels share rows as the screen grows, and prose stays capped inside panels. |
